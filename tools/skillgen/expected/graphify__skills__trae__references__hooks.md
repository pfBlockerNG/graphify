...
```bash
$(cat graphify-out/.graphify_python) -m graphify trae install       # or: graphify trae-cn install
```
...
```bash
$(cat graphify-out/.graphify_python) -m graphify trae uninstall     # or: graphify trae-cn uninstall   # remove the section
```
